...
class SelfAttention_KVCache(nn.Module):
    ...
    def forward(
        ...
        roped_query = rope_apply(q, grid_sizes, freqs, start_frame=current_start_frame).type_as(v)
        roped_key = rope_apply(k, grid_sizes, freqs, start_frame=current_start_frame).type_as(v)
...
class CrossAttention_KVCache(nn.Module):
    def __init__(
        self,
        dim,
        num_heads,
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.self_attn = SelfAttention_KVCache(
            dim=dim, 
            num_heads=num_heads, 
        ...
        self.cross_attn = CrossAttention_KVCache(
            dim=dim, 
            num_heads=num_heads, 
            window_size=(-1, -1), 
            qk_norm=qk_norm, 
    ...
    def forward(
        ...
        Note: `frame_seqlen`, `cross_attn_first_call` and `seq_lens_int` are
        accepted only for compatibility with the shared sequence-parallel
        forward (`sp_dit_forward_causal`), which passes them for the fast
        model. They are ignored here; the equivalent values are derived
        internally.
        """
...
class WanModelCausal(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.patch_embedding_wancamctrl = nn.Linear(6 * 64 * patch_size[0] * patch_size[1] * patch_size[2], dim)
